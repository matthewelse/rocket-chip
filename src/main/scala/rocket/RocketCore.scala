...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  if (usingMacroFusion) {
    ...
    .elsewhen (fuse_add) {
      // shift then add
      id_op1_shamt := id_inst(0)(21,20)
      id_ctrl.rxs2 := true.B
      id_ctrl.sel_alu2 := A2_RS2
      id_ctrl.alu_fn := ALU.FN_ADD
    }
  }
  ...
  val id_csr_ren = id_ctrl.csr.isOneOf(CSR.S, CSR.C) && id_raddr1 === 0.U
  ...
  val id_scie_decoder = if (!rocketParams.useSCIE) WireInit(new SCIEDecoderInterface, DontCare) else {
  ...
  val ex_op2_mask = RegNext(id_op2_mask)
  val ex_op2 = Mux(ex_op2_mask, ex_op2_fused, ex_op2_normal)
  ...
  val ex_sfence = usingVM.B && ex_ctrl.mem && ex_ctrl.mem_cmd === M_SFENCE
  ...
  csr.io.pc := wb_reg_pc
  val tval_valid = wb_xcpt && wb_cause.isOneOf(Causes.illegal_instruction.U, Causes.breakpoint.U,
  ...
  // gate the clock
  val unpause = csr.io.time(rocketParams.lgPauseCycles-1, 0) === 0.U || io.dmem.perf.release || take_pc
  ...
  def coverExceptions(exceptionValid: Bool, cause: UInt, labelPrefix: String, coverCausesLabels: Seq[(Int, String)]): Unit = {
    for ((coverCause, label) <- coverCausesLabels) {
      cover(exceptionValid && (cause === coverCause.U), s"${labelPrefix}_${label}")
    }
  }
  ...
  def canFuseShifts(i1: ExpandedInstruction, i2: ExpandedInstruction) : Bool = {
  ...
  def canFuseShiftAdd(i1: ExpandedInstruction, i2: ExpandedInstruction): Bool = {
    ...
    val can_fuse = canFuseChoices(shifts_left, adds, i1, i2)

    // check immediates
    val imm = i1.bits(24, 20)
    val immediates = (imm <= 3.U) && (imm >= 1.U)
