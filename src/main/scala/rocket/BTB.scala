...
import freechips.rocketchip.diplomaticobjectmodel.model.OMRocketBranchPredictor
...
class BTB(implicit p: Parameters) extends BtbModule {
  ...
  if (btbParams.bhtParams.nonEmpty) {
    ...
    val omBTB = OMRocketBranchPredictor(
      nBtbEntries = btbParams.nEntries,
      nBhtEntries = btbParams.bhtParams.map { case b => b.nEntries}.getOrElse(0),
      nRasEntries = btbParams.nRAS
    )
