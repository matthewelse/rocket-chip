...
import freechips.rocketchip.diplomaticobjectmodel.model
...
class RocketTile(
  ...
  val cpuDevice = new SimpleDevice("cpu", Seq("sifive,rocket0", "riscv")) {
    ...
    def getOMCabooseCores(resourceBindingsMap: ResourceBindingsMap): Seq[OMRocketCore] = { // TODO use resourceBindingsMap: ResourceBindingsMap?
      ...
      val mulDiv = coreParams.mulDiv.map { md => MulDivOM.makeOMI(md, xLen) }

      val baseInstructionSet = xLen match {
        case 32 => if (XLen == 32) RV32E else RV32I // TODO Review
        case 64 => RV64I
      ...
      val baseISAVersion = ISAExtensions.baseISASpecification(baseInstructionSet)
      ...
      Seq(model.OMRocketCore(
        ...
        branchPredictor = None, // Option[OMRocketBranchPredictor],
        dcache = None, // Option[OMDCache],
